...
class sql_orderMod extends commonMod{
	...
	*/
	public function get_order($a_id, $url, $status = '', $sender = 0, $school = 0, $brand = 0, $ex_d = '', $pr_d = ''){
		$con = 'a_id='. $a_id;

		if (!empty($status)) {
			if ($status == 'WAIT_SELLER_SEND_GOODS_MONEY') {
				$con .= ' and status = \'WAIT_SELLER_SEND_GOODS_MONEY\'';
			}else if ($status == 'TRADE_CLOSED') {
				$con .= ' and status = \'TRADE_CLOSED\'';
			}
		}
		...
		$field = 'o_id, addr, name, phone, created, s_name, detail, pay_type, status, send_id, send_name';
